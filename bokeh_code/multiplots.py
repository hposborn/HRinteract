...
#Adding Absolute Magnitude to columns to sort by:
...
#Adding stand-in column values to represent color and sizes:
df['size']=np.tile(1,len(df))
df['col']=np.tile('',len(df))

all_source = ColumnDataSource(data=df)

#Initialising the "selection" menus and the age slider:
...
#Making an initial view (according to age):
...
def update_source_filter():
    #Updating the "filter" (view) on the data source according to age:
...
def create_figure1(view):
    ...
    #Adding info to be shown when hovering over a star:
    TOOLTIPS = [
    ("Mass", "@star_mass"),
    ("log(Radius)", "@log_R"),
    ("log(Teff)", "@log_Teff"),
    ...
    #Flipping log_Teff, BolMag, log_G which are typically inverted
    for col in ['log_Teff', 'log_G', 'AbsMag']:
        if col==x.value:
